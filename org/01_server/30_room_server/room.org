...
#+hugo_section: docs
#+hugo_bundle: 01_server/30_room_server/room
...
* room new
  ...
  enter table，但没有sit on chair就表示旁观



  table自身拆分为一系列小的组合. 比如游戏结束的处理就专门给sub_gameend.
  table 只负责流程， 具体处理扔给后面的handler


  ready 要不要是与椅子的交互呢？
  table肯定是enter和leave 两种动作
  chair肯定也有sit和up
  如果ready不是与椅子的交互，又是什么呢？
